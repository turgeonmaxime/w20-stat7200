...
## Geometric interpretation of PCA {.allowframebreaks}
  ...
  - First, we can treat $W_k$ constant and minimise over $\mu, \mathbf{Z}_i$. Write 
  $$\phi(\mu, \mathbf{Z}_i) = \sum_{i=1}^n \| \mathbf{Y}_i - \mu - W_k \mathbf{Z}_i \|_2^2.$$
  - We will take the derivative of $\phi$ with respect to both $\mu$ and $\mathbf{Z}_i$. Using the chain rule (and remembering that the derivatives should be a $p$- and a $k$-dimension column vector, respectively), we get:
  \begin{align}
  \frac{\partial}{\partial \mu} \phi(\mu, \mathbf{Z}_i) &= \sum_{i=1}^n -2(\mathbf{Y}_i - \mu - W_k \mathbf{Z}_i), \label{eqn:mu}\\
   \frac{\partial}{\partial \mathbf{Z}_i} \phi(\mu, \mathbf{Z}_i) &=  -2W_k^T(\mathbf{Y}_i - \mu - W_k \mathbf{Z}_i).\label{eqn:zi}
  \end{align}
  - Equation \ref{eqn:mu} gives us
  ...
  - Similarly, Equation \ref{eqn:zi} gives us
  ...
  \begin{align*}
  \bar{\mathbf{Z}} &= W_k^T(\bar{\mathbf{Y}} - \mu)\\
    &= W_k^T(\bar{\mathbf{Y}} - \bar{\mathbf{Y}} + W_k\bar{\mathbf{Z}})\qquad \mbox{(since $\mu = \bar{\mathbf{Y}} - W_k\bar{\mathbf{Z}}$)}\\
    &= W_k^T(W_k\bar{\mathbf{Z}})\\
    &= \bar{\mathbf{Z}}.
  \end{align*}
  ...
  $$ \min_{W_k} \sum_{i=1}^n \| (\mathbf{Y}_i - \mathbf{\bar{Y}}) - W_k W_k^T(\mathbf{Y}_i - \mathbf{\bar{Y}}) \|_2^2.$$
...
<!-- ## Proof (EY Theorem) {.allowframebreaks} -->
