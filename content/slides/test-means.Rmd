...
## Minimum Covariance Determinant Estimator {.allowframebreaks}
  ...
  - However, there is cost in efficiency. This is can be counterbalanced by *reweighting* the estimators:
    + Let $d_i^2 = (\mathbf{Y} - \bar{\mathbf{Y}}_{MCD})^TS_{MCD}^{-1}(\mathbf{Y} - \bar{\mathbf{Y}}_{MCD})$ be the Mahalanobis distances under the original MCD estimate.
...
## Robust $T^2$ test statistic {.allowframebreaks}
  ...
  $$T^2_{MCD} = n(\mathbf{Y} - \bar{\mathbf{Y}}_{R})^TS_{R}^{-1}(\mathbf{Y} - \bar{\mathbf{Y}}_{R}).$$
