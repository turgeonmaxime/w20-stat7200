...
## Proof {.allowframebreaks}
  ...
  - First, if we write $\Sigma = LL^T$, we can replace $\mathbf{Y}$ by $L^{-1}\mathbf{Y}$ and $W$ with $L^TWL$ without changing $T^2$.
  ...
  \frac{1}{m}T^2 &= \mathbf{Y}^T W^{-1}\mathbf{Y}\\
    &= (H\mathbf{Y})^T (H^TWH)^{-1}(H^T\mathbf{Y})\\
    &= (H\mathbf{Y})^T (V)^{-1}(H^T\mathbf{Y})\\
    &= (\mathbf{Y}^T\mathbf{Y})^{1/2} v^{-1}_{11\mid 2}(\mathbf{Y}^T\mathbf{Y})^{1/2}\\
    &= (\mathbf{Y}^T\mathbf{Y})/v_{11\mid 2}.
  \end{align*}
  - In other words, we have expressed $\frac{1}{m}T^2$ as a ratio of chi-squares. 
  - Therefore, we have
  \begin{align*}
